...
load.number.passenger <- function() {
  query <-  "
    SELECT COUNT(DISTINCT r.passenger_id) AS liczba_pasazerow_dzisiaj
    FROM reservations r
...
load.past.arrivals <- function() {
  ...
  arrivals_data$arrival_time <- format(as.POSIXct(arrivals_data$arrival_time,origin = "1970-01-01", tz = "UTC"), "%Y-%m-%d %H:%M:%S")
  return(arrivals_data) 
}

# PAST DEPARTURES--------------------------------------------------------------

load.past.departures <- function() {
  ...
  departures_data$departure_time <- format(as.POSIXct(departures_data$departure_time,origin = "1970-01-01", tz = "UTC"), "%Y-%m-%d %H:%M:%S")
  departures_data$gate_opening_time <- format(as.POSIXct(departures_data$gate_opening_time,origin = "1970-01-01", tz = "UTC"), "%Y-%m-%d %H:%M:%S")
...
load.all.flights <- function() {
  ...
  flights_data$departure_time <- format(
    as.POSIXct(flights_data$departure_time,
               origin = "1970-01-01", tz = "UTC"),
    "%Y-%m-%d %H:%M:%S"
  )
  flights_data$arrival_time <- format(
    as.POSIXct(flights_data$arrival_time,
               origin = "1970-01-01", tz = "UTC"),
    "%Y-%m-%d %H:%M:%S"
  )
...
load.filtered.flights <- function(departure_airport = NULL, airlines = NULL, date = NULL) {
  ...
  flights_data <- as.data.frame(flights_data)
  flights_data$arrival_time <- format(as.POSIXct(flights_data$arrival_time,origin = "1970-01-01", tz = "UTC"), "%Y-%m-%d %H:%M:%S")
...
load.filtered.departures <- function(destination = NULL, airlines = NULL, date = NULL) {
  ...
  flights_data <- as.data.frame(flights_data)
  flights_data$departure_time <- format(as.POSIXct(flights_data$departure_time,origin = "1970-01-01", tz = "UTC"), "%Y-%m-%d %H:%M:%S")
  flights_data$gate_opening_time <- format(as.POSIXct(flights_data$gate_opening_time,origin = "1970-01-01", tz = "UTC"), "%Y-%m-%d %H:%M:%S")
...
assign.gate <- function(flight_id, gate_id) {
  con <- open.my.connection()
  on.exit(close.my.connection(con), add = TRUE)
  count <- dbGetQuery(con, sprintf(
    "SELECT COUNT(*) AS cnt FROM gateassignments WHERE flight_id = %d;",
    as.integer(flight_id)
  ))$cnt
  if (count > 0) {
    return("This flight already has a gate assigned.")
  }
  
  query <- sprintf(
    "INSERT INTO gateassignments (flight_id, gate_id, gate_opening_time) VALUES (%d, %d, CURRENT_TIMESTAMP);",
    as.integer(flight_id), as.integer(gate_id)
  )
  tryCatch({
    dbExecute(con, query)
    "Gate assigned successfully!"
  }, error = function(e) {
    message("Error assigning gate: ", e$message)
    e$message
  })
}
...
load_flight_details<- function(flight_id) {
  ...
  flights_details_data$arrival_time <- format(as.POSIXct(flights_details_data$arrival_time,origin = "1970-01-01", tz = "UTC"), "%Y-%m-%d %H:%M:%S")
  flights_details_data$departure_time <- format(as.POSIXct(flights_details_data$departure_time,origin = "1970-01-01", tz = "UTC"), "%Y-%m-%d %H:%M:%S")
...
update_flight_details <- function(flight_id, departure_time = NULL, arrival_time = NULL, 
  ...
  if (!is.null(departure_time)) {
    sets <- c(sets, sprintf("departure_time = '%s'", as.character(departure_time)))
  }
  if (!is.null(arrival_time)) {
    sets <- c(sets, sprintf("arrival_time = '%s'", as.character(arrival_time)))
  }
  if (!is.null(aircraft_id)) {
    sets <- c(sets, sprintf("aircraft_id = %d", as.integer(aircraft_id)))
  }
  if (!is.null(status)) {
    sets <- c(sets, sprintf("status = '%s'", status))
  }
  ...
  query <- sprintf(
    "UPDATE flights SET %s WHERE flight_id = %d;",
    paste(sets, collapse = ", "),
    as.integer(flight_id)
  )
  ...
  tryCatch({
    ...
    message("Error updating flight details: ", e$message)
    FALSE
  })
}
...
add_new_flight <- function(destination_id, departure_time, arrival_time) {
  ...
  tryCatch({
    ...
    message("Error inserting flight: ", e$message)
    return(FALSE)
  })
}
...
load.cancelled.flights.table <- function() {
  ...
  cancelled_flights$arrival_time <- format(as.POSIXct(cancelled_flights$arrival_time,origin = "1970-01-01", tz = "UTC"), "%Y-%m-%d %H:%M:%S")
  cancelled_flights$departure_time <- format(as.POSIXct(cancelled_flights$departure_time,origin = "1970-01-01", tz = "UTC"), "%Y-%m-%d %H:%M:%S")
...
add_reservation_to_db <- function(flight_id, passenger_id) {
  query <- sprintf(
    "INSERT INTO reservations (flight_id, passenger_id) VALUES (%d, %d);",
    as.integer(flight_id),
    as.integer(passenger_id)
  )
  con <- open.my.connection()
  on.exit(close.my.connection(con), add = TRUE)
  tryCatch({
    ...
    message("Error adding reservation: ", e$message)
    e$message
  })
}
...
add_passenger_to_db <- function(first_name, last_name, document_number, date_of_birth, gender, email, phone_number) {
  email_val <- ifelse(nzchar(email), paste0("'", email, "'"), "NULL")
  phone_val <- ifelse(nzchar(phone_number), paste0("'", phone_number, "'"), "NULL")
  query <- sprintf(
    "INSERT INTO passengers (first_name, last_name, document_number, date_of_birth, gender, email, phone_number) VALUES ('%s', '%s', '%s', '%s', '%s', %s, %s);",
    first_name, last_name, document_number, date_of_birth, gender, email_val, phone_val
  )
  con <- open.my.connection()
  on.exit(close.my.connection(con), add = TRUE)
  tryCatch({
    ...
    message("Error adding passenger: ", e$message)
    if (grepl("UNIQUE constraint", e$message)) "Duplicate document" else e$message
  })
}
...
add_luggage_to_db <- function(reservation_id, weight) {
  query <- sprintf(
    "INSERT INTO luggage (reservation_id, weight, status) VALUES (%d, %.2f, NULL);",
    as.integer(reservation_id), as.numeric(weight)
  )
  con <- open.my.connection()
  on.exit(close.my.connection(con), add = TRUE)
  tryCatch({
    ...
    message("Error adding luggage: ", e$message)
    e$message
  })
}
...
update_luggage_status_in_db <- function(luggage_id, new_status) {
  query <- paste(
...
add_aircraft_to_airline <- function(model, seats, range, fuel_consumption, fuel_type, airline_name) {
  query <- sprintf(
    "INSERT INTO aircrafts (model, seats, range, fuel_consumption, fuel_type, airline_id) VALUES ('%s', %d, %d, %f, '%s', (SELECT airline_id FROM airlines WHERE name = '%s'));",
    model, as.integer(seats), as.integer(range), as.numeric(fuel_consumption), fuel_type, airline_name
  )
  con <- open.my.connection()
  on.exit(close.my.connection(con), add = TRUE)
  tryCatch({
...
add_aircraft_to_wizzair <- function(model, seats, range, fuel_consumption, fuel_type) {
  add_aircraft_to_airline(model, seats, range, fuel_consumption, fuel_type, "Wizz Air")
}
add_aircraft_to_lot <- function(model, seats, range, fuel_consumption, fuel_type) {
  add_aircraft_to_airline(model, seats, range, fuel_consumption, fuel_type, "LOT")
}
add_aircraft_to_ryanair <- function(model, seats, range, fuel_consumption, fuel_type) {
  add_aircraft_to_airline(model, seats, range, fuel_consumption, fuel_type, "Ryanair")
}
...
get.longest.serving.employee.years <- function() {
  ...
  if (is.numeric(oldest_date)) {
    
    oldest_date <- as.Date(oldest_date, origin = "1970-01-01")
  } else {

    oldest_date <- as.Date(oldest_date)
  }
  
  days_worked <- as.numeric(difftime(Sys.Date(), oldest_date, units = "days"))
  years_worked <- floor(days_worked / 365.25)
  
  return(as.integer(years_worked))
}
...
load.employee.list <- function() {
  ...
  if (is.numeric(employee_list$hire_date)) {
    employee_list$hire_date <- format(
      as.Date(employee_list$hire_date, origin = "1970-01-01"),
      "%Y-%m-%d"
    )
  } else {
    
    employee_list$hire_date <- as.character(employee_list$hire_date)
  }
